...
post '/login' do
  host = 'smashconvention.basecamphq.com'
  begin
    Basecamp.establish_connection! host, params[:username], params[:password], true
    user = Basecamp::Person.me
    session[:username] = params[:username] unless user.nil?
    session[:fullname] = user.attributes['name'] unless user.nil?
